  ...
  var Emoji = {
    ...
    getEmojiHTML: function(code, symbol, enabled, is_tab) {
      ...
      if (Emoji.cssEmoji[code] != undefined) {
        ...
        return '<img'+editable+' src="//vk.com/images/blank.gif" emoji="'+code+'" '+(symbol ? 'alt="'+symbol+'"' : symbol)+' class="emoji_css" style="background-position: 0px '+num+'px;" />';
      } else {
        if (!Emoji.imgEmoji[code] && symbol && !enabled) {
          return symbol;
        } else if (is_tab) {
          return '<img class="emoji emoji_need_load" '+(symbol ? 'alt="'+symbol+'"' : '')+' src="http://vk.com/images/blank.gif" data-src="http://vk.com/images/emoji/'+code+(window.devicePixelRatio >= 2 ? '_2x' : '')+'.png" />';
        } else {
          return '<img class="emoji" '+(symbol ? 'alt="'+symbol+'"' : '')+' src="//vk.com/images/emoji/'+code+(window.devicePixelRatio >= 2 ? '_2x' : '')+'.png" />';
        }
      }
    },
    ...
    getTabCont: function(optId, selId) {
      ...
      if (selId) {
        ...
        for (var i in list) {
          html += '<a id="emoji_sticker_item'+optId+'_'+selId+'_'+list[i][0]+'" class="emoji_sticker_item" onclick="Emoji.stickerClick('+optId+', '+list[i][0]+', '+list[i][1]+', this);"><img class="emoji_need_load" width="'+parseInt(list[i][1] / sizeDiv)+'" height="'+size+'" src="http://vk.com/images/blank.gif" data-src="http://vk.com/images/stickers/'+list[i][0]+'/'+stickerSize+'.png" /></a>';
        }
      } else {
    ...
    getTabsCode: function(newStickers, optId) {
      ...
      for (var i in stickers) {
        ...
        if (stNum === -1) {
          ...
          html += '<a class="emoji_tab emoji_tab_img_cont emoi_tab_'+stNum+' emoji_tab_' + stNum + (cur.stickersTab == stNum ? ' emoji_tab_sel' : '')+(isActive ? '' : ' emoji_tab_promo')+'" onclick="'+act+'"><img width="22" height="22" src="http://vk.com/images/store/stickers/'+stNum+'/thumb_'+(window.devicePixelRatio >= 2 ? '44' : '22')+'.png" class="emoji_tab_img"/></a>';
        } else {
