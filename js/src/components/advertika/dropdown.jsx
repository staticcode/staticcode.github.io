...
const Dropdown = React.createClass({
  ...
  eachColumnItems(columns, callback) {
    switch (this.props.type) {
      case 'geo':
        return columns[1].forEach(c => c.items.forEach(callback)
      );
      case 'groups':
        return columns.forEach(c => c.items.forEach(callback)
      );
      default:
        return columns.forEach(c => c.forEach(c => c.items ? c.items.forEach(callback) : null)
      );
    }
  },
